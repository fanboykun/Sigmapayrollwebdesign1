-- ============================================================================
-- UPDATE CLINIC NURSES PERMISSIONS
-- ============================================================================
-- Version: 1.0.0
-- Description: Memastikan semua role memiliki permissions yang tepat untuk
--              modul clinic_master_nurses yang terintegrasi dengan data karyawan
-- Date: 2025-11-10
-- ============================================================================

-- This migration ensures all roles have proper permissions for the
-- clinic_master_nurses module
...
BEGIN
  ...
  -- SUPER ADMIN - Full access to clinic_master_nurses
  ...
  -- CLINIC ADMIN - Can manage nurses (no delete)
  INSERT INTO role_permissions (role_id, module_name, can_view, can_create, can_edit, can_delete)
  VALUES
  ...
  -- CLINIC DOCTOR - View only (can see nurse information)
  IF v_doctor_id IS NOT NULL THEN
    INSERT INTO role_permissions (role_id, module_name, can_view, can_create, can_edit, can_delete)
    VALUES
      (v_doctor_id, 'clinic_master_nurses', true, false, false, false)
    ON CONFLICT (role_id, module_name)
    DO UPDATE SET
      can_view = true,
      can_create = false,
  ...
  -- CLINIC NURSE - View only (can see other nurses)
  IF v_nurse_id IS NOT NULL THEN
    INSERT INTO role_permissions (role_id, module_name, can_view, can_create, can_edit, can_delete)
    VALUES
      (v_nurse_id, 'clinic_master_nurses', true, false, false, false)
    ON CONFLICT (role_id, module_name)
    DO UPDATE SET
      can_view = true,
      can_create = false,
  ...
  -- Log completion
  RAISE NOTICE 'Clinic nurses permissions updated successfully';
...
WHERE rp.module_name = 'clinic_master_nurses'
ORDER BY r.name;
*/
